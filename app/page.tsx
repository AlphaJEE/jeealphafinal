    ...
    <div className="home-premium relative min-h-screen overflow-hidden">
        ...
        <div className="max-w-[1400px] mx-auto px-5 sm:px-8 py-16 grid grid-cols-2 md:grid-cols-4 gap-y-10">
          {stats.map((s, i) => (
            <Reveal key={i} delay={i * 90} className="text-center">
              <p className="font-season text-[clamp(34px,4vw,56px)] font-semibold text-[#0f0e0b] leading-none tabular-nums">
                <CountUp to={s.to} duration={1700} />
                <span className="text-[#9d937c]">{s.suffix}</span>
              </p>
              <p className="font-akkurat text-[10px] uppercase tracking-[0.2em] text-[#3d3b34] mt-3">
                {s.label}
              </p>
            </Reveal>
          ))}
      ...
      <section className="relative max-w-[1400px] mx-auto px-5 sm:px-8 py-24">
        <Reveal className="mb-14 max-w-xl">
          ...
          <h2 className="font-season text-[clamp(34px,5vw,64px)] leading-[0.98] text-[#0f0e0b]">
          ...
          {features.map((f, i) => (
            <Reveal key={i} delay={(i % 3) * 90}>
              <SpotlightCard className="glass rounded-2xl p-8 h-full" tilt={false}>
                ...
                <h3 className="font-season text-2xl text-[#0f0e0b] mb-3">{f.label}</h3>
                <p className="font-season text-[#9d937c] leading-relaxed">{f.desc}</p>
              </SpotlightCard>
            </Reveal>
          ))}
        </div>
      ...
      <section className="relative bg-[#0f0e0b] py-28">
        <div className="relative z-10 max-w-[1400px] mx-auto px-5 sm:px-8 text-center">
          <Reveal>
            <p className="font-akkurat text-[11px] uppercase tracking-[0.25em] text-[#9d937c] mb-5">
              Community
            </p>
            <h2 className="font-season text-[clamp(30px,4vw,58px)] leading-[1] text-[#f9f9f0] mb-6">
              Join serious aspirants.
            </h2>
            <p className="font-season text-xl text-[#9d937c] max-w-xl mx-auto mb-10">
